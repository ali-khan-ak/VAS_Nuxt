  ...
  <div class="row">
    ...
    <div class="col-12 mb-3 mt-3">
      <card class="filter-card">
        ...
        <!-- Service Type Toggle — NEW: SMS vs USSD selector -->
        <div class="d-flex align-items-center mb-3" style="gap: 8px;">
          <span class="service-toggle-label">Report Type:</span>
          <div class="service-toggle">
            <button
              class="svc-btn"
              :class="{ 'svc-active': serviceType === 'all' }"
              @click="setServiceType('all')"
              type="button"
            >
              <i class="tim-icons icon-bullet-list-67 mr-1"></i> All
            </button>
            <button
              class="svc-btn"
              :class="{ 'svc-active': serviceType === 'sms' }"
              @click="setServiceType('sms')"
              type="button"
            >
              <i class="tim-icons icon-send mr-1"></i> SMS Only
            </button>
            <button
              class="svc-btn"
              :class="{ 'svc-active': serviceType === 'ussd' }"
              @click="setServiceType('ussd')"
              type="button"
            >
              <i class="tim-icons icon-mobile mr-1"></i> USSD Only
            </button>
          </div>
        </div>
    ...
    <!-- ══ REPORT TABS — filtered by serviceType ══ -->
    <div class="col-12 mb-3">
      <div class="report-tabs">
        ...
        >
          ...
          <span class="tab-service-badge" :class="tab.service === 'sms' ? 'tsb-sms' : 'tsb-ussd'">
            {{ tab.service.toUpperCase() }}
          </span>
        </button>
      </div>
      <!-- Empty state when service filter hides all tabs -->
      <div v-if="visibleTabs.length === 0" class="no-tabs-msg">
        <i class="tim-icons icon-alert-circle-exc mr-2"></i>
        No tabs match the current service filter. Switch to "All" or select a different type.
      </div>
    </div>
      ...
      <div class="col-12">
        <card card-body-classes="table-full-width">
          <div slot="header" class="d-flex justify-content-between align-items-center">
            ...
            <span class="svc-pill svc-pill-sms">SMS</span>
          </div>
          <el-table :data="filteredOutbound" style="width:100%">
            ...
            <el-table-column min-width="150" sortable label="MSISDN / Segment" prop="msisdn">
              <template slot-scope="{ row }"><span class="mono-text">{{ row.msisdn }}</span></template>
            </el-table-column>
        ...
        <card card-body-classes="table-full-width">
          ...
          <el-table :data="filteredInbound" style="width:100%">
            ...
            <el-table-column min-width="110" sortable label="Shortcode" prop="shortcode">
              <template slot-scope="{ row }"><span class="code-tag code-blue">{{ row.shortcode }}</span></template>
            </el-table-column>
            <el-table-column min-width="110" sortable label="Keyword" prop="keyword">
              <template slot-scope="{ row }"><span class="code-tag code-purple">{{ row.keyword }}</span></template>
            </el-table-column>
            <el-table-column min-width="200" label="Message" prop="message">
              <template slot-scope="{ row }"><span class="msg-preview">{{ row.message }}</span></template>
            </el-table-column>
      ...
      <div class="col-12">
        <card card-body-classes="table-full-width">
          <div slot="header" class="d-flex justify-content-between align-items-center">
            ...
            <span class="svc-pill svc-pill-ussd">USSD</span>
          </div>
          <el-table :data="filteredUssd" style="width:100%">
            ...
            <el-table-column min-width="110" sortable label="Duration" prop="duration">
              <template slot-scope="{ row }">{{ row.duration }}s</template>
            </el-table-column>
            <el-table-column min-width="90"  sortable label="Steps"  prop="steps"></el-table-column>
...
const ALL_TABS = [
  { key: 'sms_outbound',  label: 'SMS Outbound',  icon: 'icon-send',    count: 284540, service: 'sms'  },
  { key: 'sms_inbound',   label: 'SMS Inbound',   icon: 'icon-chat-33', count: 18320,  service: 'sms'  },
  { key: 'ussd_sessions', label: 'USSD Sessions', icon: 'icon-mobile',  count: 43810,  service: 'ussd' },
];
...
export default {
  ...
  data() {
    return {
      ...
      ussdStats: {
        successful: 42170, failed: 980, timeout: 660,
        avgDuration: 28, subFlow: 14820, unsubFlow: 3240, directFlow: 25750
      },
      ...
      outboundData: [
        ...
        { id:4, campaign:'Fantasy Football Reminder',   type:'Broadcast',       msisdn:'98,000 subscribers',  status:'Pending',   sent:'0',       delivered:'0',       failed:'0',      deliveryRate:0,  date:'2025-03-10 09:00' },
        { id:5, campaign:'Serie A Weekend Preview',     type:'Selective',       msisdn:'45,000 subscribers',  status:'Failed',    sent:'12,100',  delivered:'3,146',   failed:'8,954',  deliveryRate:26, date:'2025-03-04 07:00' },
        ...
        { id:7, campaign:'La Liga GW26 Results',        type:'Selective',       msisdn:'75,000 subscribers',  status:'Delivered', sent:'74,100',  delivered:'72,618',  failed:'1,482',  deliveryRate:98, date:'2025-02-25 20:00' },
      ],
  ...
  computed: {
    visibleTabs() {
      if (this.serviceType === 'all') return ALL_TABS;
      return ALL_TABS.filter(t => t.service === this.serviceType);
    },
    filteredOutbound() {
      return this.outboundData.filter(r => {
        const s = this.globalSearch.toLowerCase();
        return (!s || r.campaign.toLowerCase().includes(s) || r.msisdn.toLowerCase().includes(s))
          && (!this.outboundFilters.status   || r.status   === this.outboundFilters.status)
          && (!this.outboundFilters.campaign || r.campaign === this.outboundFilters.campaign)
          && (!this.outboundFilters.type     || r.type     === this.outboundFilters.type);
      });
    },
    filteredInbound() {
      return this.inboundData.filter(r => {
        const s = this.globalSearch.toLowerCase();
        return (!s || r.msisdn.toLowerCase().includes(s) || r.keyword.toLowerCase().includes(s) || r.message.toLowerCase().includes(s))
          && (!this.inboundFilters.shortcode || r.shortcode === this.inboundFilters.shortcode)
          && (!this.inboundFilters.keyword   || r.keyword   === this.inboundFilters.keyword)
          && (!this.inboundFilters.routed    || r.status    === this.inboundFilters.routed);
      });
    },
    filteredUssd() {
      return this.ussdData.filter(r => {
        const s = this.globalSearch.toLowerCase();
        return (!s || r.msisdn.toLowerCase().includes(s) || r.flow.toLowerCase().includes(s))
          && (!this.ussdFilters.status    || r.status    === this.ussdFilters.status)
          && (!this.ussdFilters.flow      || r.flow      === this.ussdFilters.flow)
          && (!this.ussdFilters.shortcode || r.shortcode === this.ussdFilters.shortcode);
      });
    },
  ...
  methods: {
    setServiceType(type) {
      ...
      // Auto-select the first visible tab when switching
      const visible = type === 'all' ? ALL_TABS : ALL_TABS.filter(t => t.service === type);
      if (visible.length && !visible.find(t => t.key === this.activeTab)) {
        this.activeTab = visible[0].key;
      }
    },
    ...
    applyFilters() { this.$notify({ type: 'info', message: 'Filters applied.' }); },
    clearFilters() {
      this.globalSearch = '';
    ...
    exportReport() { this.$notify({ type: 'success', message: 'Report exported as CSV.' }); },
...
.filter-divider { height:1px; background:rgba(255,255,255,0.07); }
...
/* ══ Service Type Toggle — NEW ══ */
.service-toggle-label { font-size:0.72rem; font-weight:700; text-transform:uppercase; letter-spacing:0.6px; color:#9a9a9a; white-space:nowrap; }
.service-toggle { display:flex; gap:4px; background:rgba(255,255,255,0.04); border:1px solid rgba(255,255,255,0.1); border-radius:10px; padding:4px; }
.svc-btn {
  background:transparent; border:none; border-radius:7px;
  color:#9a9a9a; font-size:0.78rem; font-weight:600;
  padding:6px 16px; cursor:pointer; transition:all 0.2s;
  display:flex; align-items:center; white-space:nowrap;
}
.svc-btn:hover { color:#fff; background:rgba(255,255,255,0.06); }
.svc-active { background:rgba(225,78,202,0.15) !important; color:#e14eca !important; }

/* ══ Report Tabs ══ */
.report-tabs { display:flex; gap:6px; border-bottom:1px solid rgba(255,255,255,0.08); flex-wrap:wrap; }
.report-tab-btn {
  background:transparent; border:1px solid transparent; border-bottom:none;
  border-radius:8px 8px 0 0; color:#9a9a9a; font-size:0.8rem; font-weight:600;
...
.tab-active { background:rgba(225,78,202,0.1) !important; border-color:rgba(255,255,255,0.1) !important; color:#e14eca !important; }
...
.tab-service-badge { font-size:0.6rem; font-weight:700; letter-spacing:0.8px; padding:1px 6px; border-radius:6px; }
.tsb-sms  { background:rgba(29,140,248,0.12);  color:#1d8cf8; border:1px solid rgba(29,140,248,0.2); }
.tsb-ussd { background:rgba(0,242,195,0.12);   color:#00f2c3; border:1px solid rgba(0,242,195,0.2); }

/* No-tabs message */
.no-tabs-msg { padding:16px 0; color:#9a9a9a; font-size:0.82rem; display:flex; align-items:center; }

/* ══ Table card header pill ══ */
.svc-pill { font-size:0.68rem; font-weight:700; letter-spacing:0.8px; padding:3px 10px; border-radius:10px; }
.svc-pill-sms  { background:rgba(29,140,248,0.12);  color:#1d8cf8; border:1px solid rgba(29,140,248,0.2); }
.svc-pill-ussd { background:rgba(0,242,195,0.12);   color:#00f2c3; border:1px solid rgba(0,242,195,0.2); }
...
.ussd-bar-track  { height:8px; border-radius:6px; overflow:hidden; display:flex; background:rgba(255,255,255,0.05); }
