<template>
  <div class="row">
    ...
    <!-- FIX 1: "Running" → "In Progress Campaigns" -->
    <div class="col-lg-3 col-sm-6 d-flex">
      <card class="card-stats stat-card w-100">
        <div class="stat-card-body">
          ...
          <div class="stat-content">
            <p class="stat-label">In Progress Campaigns</p>
    ...
    <div class="col-12 mb-3 mt-3">
      <card>
        <div class="row align-items-center">
          ...
          <div class="col-md-3">
            <!-- FIX 4 & 5: Scheduled → Broadcast, Broadcast → Selective -->
    ...
    <div class="col-12">
      <card card-body-classes="table-full-width">
        ...
        <el-table :data="filteredCampaigns" style="width: 100%">
          ...
          <el-table-column min-width="110" sortable label="Delivered %" prop="deliveryRate">
            <template slot-scope="{ row }">
          ...
          <el-table-column min-width="180" label="Actions" align="right" header-align="right">
            <template slot-scope="{ row }">
              <base-button
                ...
                @click="stopCampaign(row)" title="Stop"
              >
              ...
              <base-button
                ...
                @click="startCampaign(row)" title="Start"
              >
    ...
    <modal :show.sync="showCreateModal" class="modal-black" :show-close="true" header-classes="justify-content-center">
      ...
      <div class="row">
        ...
        <!-- FIX 4 & 5: Renamed types -->
        <div class="col-12 mb-3">
          <label class="control-label">Campaign Type</label>
          <el-select v-model="newCampaign.type" class="w-100">
            <el-option label="Broadcast (Send to Whole Base)" value="Broadcast"></el-option>
            <el-option label="Selective (CSV Upload)"         value="Selective"></el-option>
            <el-option label="Attribute-Based Filter"         value="Attribute-Based"></el-option>
          </el-select>
        </div>

        <!-- Broadcast: no extra input needed, just message + sender -->

        <!-- Selective: CSV Upload -->
        <div class="col-12 mb-3" v-if="newCampaign.type === 'Selective'">
        ...
        <!-- FIX 2: Single MSISDN test target toggle -->
        <div class="col-12 mb-3">
          <label class="control-label d-block mb-2">Recipient Target</label>
          <div class="send-timing-toggle mb-3">
            <button
              class="timing-btn"
              :class="{ 'timing-active': newCampaign.targetMode === 'all' }"
              @click="newCampaign.targetMode = 'all'"
              type="button"
            >
              <i class="tim-icons icon-world mr-2"></i> Full Campaign
            </button>
            <button
              class="timing-btn"
              ...
              type="button"
            >
              <i class="tim-icons icon-single-02 mr-2"></i> Test — Single Number
            </button>
          </div>
          <div v-if="newCampaign.targetMode === 'single'">
            ...
            ></base-input>
            <small class="text-muted">Campaign will only be sent to this one MSISDN.</small>
          </div>
        </div>

        <!-- FIX 3: Send Now vs Schedule timing toggle -->
        <div class="col-12 mb-3">
        ...
        <div class="col-12 mb-3" v-if="newCampaign.sendTiming === 'schedule'">
          ...
          <small class="text-muted mt-1 d-block">
            ...
            Campaign will be sent automatically at the selected time.
          </small>
        </div>
...
export default {
  ...
  data() {
    return {
      ...
      // FIX 4 & 5: updated campaign type labels in mock data
      campaigns: [
        ...
        { id: 4, name: 'Fantasy Football Reminder',    type: 'Broadcast',      status: 'Draft',     subscribers: '98,000',  sent: '0',       deliveryRate: 0,  scheduledAt: '2025-03-10 09:00' },
        { id: 5, name: 'Serie A Weekend Preview',      type: 'Selective',      status: 'Failed',    subscribers: '45,000',  sent: '12,100',  deliveryRate: 26, scheduledAt: '2025-03-04 07:00' },
        ...
        { id: 7, name: 'La Liga GW26 Results',         type: 'Selective',      status: 'Completed', subscribers: '75,000',  sent: '74,100',  deliveryRate: 98, scheduledAt: '2025-02-25 20:00' },
      ]
    };
  },
  computed: {
  ...
  methods: {
    ...
    openCreateModal() {
      this.newCampaign = {
        name: '', type: 'Broadcast', message: '', senderId: '',
        csvFile: null, targetMode: 'all', singleMsisdn: '',
        sendTiming: 'now', scheduledAt: null,
        subscriberDateRange: null, lastChargeDateRange: null, lastActivityDateRange: null
      };
      this.showCreateModal = true;
    },
    createCampaign() {
      if (!this.newCampaign.name || !this.newCampaign.message) {
        this.$notify({ type: 'warning', message: 'Please fill in campaign name and message.' });
        return;
      }
      if (this.newCampaign.type === 'Selective' && !this.newCampaign.csvFile) {
        this.$notify({ type: 'warning', message: 'Please upload a subscriber CSV for Selective campaigns.' });
        return;
      }
      if (this.newCampaign.targetMode === 'single' && !this.newCampaign.singleMsisdn) {
        this.$notify({ type: 'warning', message: 'Please enter a target MSISDN for single-number send.' });
        return;
      }
      ...
      const scheduledAt = isNow
        ? new Date().toLocaleString()
        : new Date(this.newCampaign.scheduledAt).toLocaleString();
      const subscribers = this.newCampaign.targetMode === 'single' ? '1' : '0';
      ...
      this.$notify({
        type: 'success',
        message: isNow ? 'Campaign launched successfully!' : `Campaign scheduled for ${scheduledAt}`
      });
    },
    startCampaign(row) {
      row.status = 'Running';
    ...
    clearFilters() { this.search = ''; this.statusFilter = ''; this.typeFilter = ''; },
    refreshList()  { this.$notify({ type: 'info', message: 'Campaign list refreshed.' }); },
...
.el-date-editor .el-range-input { background:transparent; color:#fff; }
